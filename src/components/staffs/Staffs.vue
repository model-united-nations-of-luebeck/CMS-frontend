<template>
  <div>
    <v-container>
      ...
      <v-btn @click="openNewStaffDialog" fab right bottom fixed color="primary"
        ><v-icon>mdi-account-plus</v-icon></v-btn
      >
      <!-- Dialog for creating new staffs -->
      <staff-dialog ref="newStaffDialog"></staff-dialog>
      ...
      <v-card-title>
        ...
        <v-btn
          ...
          @click="openAll"
          color="primary"
          v-if="this.expanded.length < this.staffs.length"
          ><tooltipped-icon
        ...
        <v-btn
          ...
          ><tooltipped-icon
            icon="mdi-download"
            text="download data of selected staffs as JSON"
            position="top"
        /></v-btn>
        <v-btn icon color="primary" v-if="this.selected.length != 0"
          ><download-csv :data="selected">
            <tooltipped-icon
              ...
              text="download data of selected staffs as CSV"
              position="top"
            /> </download-csv
        ></v-btn>
        <v-btn icon color="primary" v-if="this.selected.length != 0"
          ><download-excel :data="selected">
            <tooltipped-icon
              ...
              text="download data of selected staffs as Excel file (xls)"
              position="top"
            /> </download-excel
        ></v-btn>
        <v-spacer></v-spacer>
      ...
      <v-data-table
        ...
        :footer-props="{
          ...
          itemsPerPageText: 'Staffs per page:',
          showCurrentPage: true,
        }"
      >
        <template v-slot:[`footer.page-text`]="props"
          >Staffs {{ props.pageStart }} - {{ props.pageStop }} of
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <v-btn
            small
            icon
            :to="{ name: 'StaffDetails', params: { id: item.id } }"
          >
            <v-icon small color="primary">
              mdi-pencil
            </v-icon>
          </v-btn>
          <v-btn small icon @click="deleteStaff(item)">
...
export default {
  components: { StaffDialog, TooltippedIcon, ToastMessage },
  name: "Staffs",
  ...
  mounted() {
    ...
    this.$http
      .get("https://munoltom.pythonanywhere.com/api/staffs/")
      .then((response) => {
  ...
  methods: {
    ...
    async deleteItemConfirm() {
      await this.$http
        .delete(
          `https://munoltom.pythonanywhere.com/api/staffs/${this.editedItem.id}/`,
          {}
        )
        .then((r) => {
          if (r.status == 204) {
