...
export const useConferenceStore = defineStore("conference", () => {
  ...
  const validationRules = {
    ...
    annualSessionRules: [
      (as) => !!as || "Annual session is required",
      (as) => as > 0 || "Annual session must be greater than 0",
    ],
    startDateRules: [
      (sd) => !!sd || "Start date is required",
      (sd) =>
        new Date(sd) <= new Date(conference.value.end_date) ||
        "Start date must be earlier than end date",
      (sd) =>
        new Date(sd).getFullYear() == conference.value.year ||
        "Start date must be in the same year as the conference",
    ],
    endDateRules: [
      (ed) => !!ed || "End date is required",
      (ed) =>
        new Date(ed) >= new Date(conference.value.start_date) ||
        "End date must be later than start date",
      (ed) =>
        new Date(ed).getFullYear() == conference.value.year ||
        "End date must be in the same year as the conference",
    ],
  ...
  async function updateConferenceSettings() {
    loading.value = true;
    await http
      .put(`conferences/${conference.value.id}/`, conference.value)
      .then((res) => {
        conference.value = res.data;
        loading.value = false;
