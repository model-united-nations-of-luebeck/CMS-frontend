...
export const useDelegatesStore = defineStore("delegates", () => {
  ...
  async function getDelegate(delegate_id) {
    //notice the singular form of the function name
    loading.value = true;
    await http
      .get(`delegates/${delegate_id}/`)
      .then((res) => {
  ...
  async function updateDelegate(delegate_id) {
    loading.value = true;
    await http
      .patch(`delegates/${delegate_id}/`, delegate.value)
      .then(() => {
  ...
  async function createDelegate(forum_id, org_id) {
    loading.value = true;
    await http
      .post("delegates/", { forum: forum_id, represents: org_id })
      .then((res) => {
  ...
  async function assignSchool(delegate_id, school_id) {
    ...
    if (delegates.value.find((delegate) => delegate.id == delegate_id).data_consent_time) {
      toast.error(
        "Delegate has already filled in data, cannot unassign from school. Please reset delegate first.",
        {
          position: toast.POSITION.BOTTOM_CENTER,
        },
      );
      ...
      loading.value = true;
      await http
        .patch(`delegates/${delegate_id}/`, { school: school_id })
        .then(() => {
          let index = delegates.value.findIndex((delegate) => delegate.id == delegate_id);
          if (index !== -1) {
        ...
        .catch((error) => {
          toast.error(
            `${school_id ? "Assigning" : "Unassigning"} School failed. Please ask admin for help.`,
            {
              position: toast.POSITION.BOTTOM_CENTER,
            },
          );
  ...
  async function changeAmbassador(delegate_id) {
    ...
    await http
      .patch(`delegates/${delegate_id}/`, { ambassador: true })
      .then(() => {
        let index = delegates.value.findIndex((delegate) => delegate.id == delegate_id);
        if (index !== -1) {
        ...
        delegates.value.forEach((delegate) => {
          if (
            ...
            delegate.represents == delegates.value[index].represents &&
            delegate.ambassador
          ) {
