...
export const useMemberOrganizationsStore = defineStore("member_organizations", () => {
  ...
  async function getMemberOrganization(member_organization_id) {
    //notice the singular form of the function name
    loading.value = true;
    await http
      .get(`member-organizations/${member_organization_id}/`)
      .then((res) => {
  ...
  async function toggleActive(member_organization_id) {
    loading.value = true;
    await http
      .patch(`member-organizations/${member_organization_id}/`, {
  ...
  async function updateMemberOrganization(member_organization_id) {
    loading.value = true;
    await http
      .patch(`member-organizations/${member_organization_id}/`, member_organization.value)
      .then(() => {
        let index = member_organizations.value.findIndex((org) => org.id == member_organization_id);
        if (index !== -1) {
          member_organizations.value[index] = {
            ...member_organizations.value[index],
            ...member_organization.value,
          };
        }
  ...
  async function createMemberOrganization() {
    loading.value = true;
    await http
      .post("member-organizations/", member_organization.value)
      .then((res) => {
  ...
  async function deleteMemberOrganization(member_organization_id) {
    await http
      .delete(`member-organizations/${member_organization_id}/`)
      .then(() => {
        member_organizations.value = member_organizations.value.filter(
          (org) => org.id != member_organization_id,
        ); // keep all member_organizations that do not have the id of the deleted member_organization
      })
      .catch((error) => {
        console.error(error);
        throw error; // rethrow the error to be caught at the point where this function is called
