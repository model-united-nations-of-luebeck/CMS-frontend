...
export const useMUNDirectorsStore = defineStore("mun-directors", () => {
  ...
  async function getMUNDirector(mun_director_id) {
    //notice the singular form of the function name
    loading.value = true;
    await http
      .get(`mun-directors/${mun_director_id}/`)
      .then((res) => {
  ...
  async function updateMUNDirector(mun_director_id) {
    loading.value = true;
    await http
      .patch(`mun-directors/${mun_director_id}/`, mun_director.value)
      .then(() => {
      ...
      .catch((error) => {
        if (error.response && error.response.status === 400 && error.response.data.email) {
          toast.error(
            "Updating MUN-Director failed. An MUN-Director with this email already exists.",
            {
              position: toast.POSITION.BOTTOM_CENTER,
            },
          );
  ...
  async function deleteMUNDirector(mun_director_id) {
    loading.value = true;
    await http
      .delete(`mun-directors/${mun_director_id}/`)
      .then(() => {
        mun_directors.value = mun_directors.value.filter(
          (mun_dir) => mun_dir.id != mun_director_id,
        ); // keep all mun_directors that do not have the id of the deleted mun_director
        loading.value = false;
      ...
      .catch((error) => {
        toast.error("Deleting MUN-Director failed. Please ask admin for help.", {
          position: toast.POSITION.BOTTOM_CENTER,
        });
        console.error(error);
        loading.value = false;
  ...
  async function createEmptyMUNDirectorForSchool(school_id) {
    loading.value = true;
    await http
      .post("mun-directors/", { school: school_id })
      .then((res) => {
      ...
      .catch((error) => {
        toast.error("Adding MUN-Director failed. Please ask admin for help.", {
          position: toast.POSITION.BOTTOM_CENTER,
        });
        console.error(error);
        loading.value = false;
  ...
  return {
    ...
    updateMUNDirector,
    deleteMUNDirector,
    createEmptyMUNDirectorForSchool,
  };
});
